...
use BookneticVendor\Gettext\Utils\FunctionsScanner;
/**
...
class VueJs extends Extractor implements ExtractorInterface, ExtractorMultiInterface
{
    ...
    protected static $functionsScannerClass = 'BookneticVendor\\Gettext\\Utils\\JsFunctionsScanner';
    /**
     * @inheritDoc
     * @throws Exception
     */
    ...
    public static function fromStringMultiple($string, array $translations, array $options = [])
    {
        ...
        $options += [
            ...
            'attributePrefixes' => [':', 'v-bind:', 'v-on:', 'v-text'],
            // HTML Tags to parse
            'tagNames' => ['translate'],
            // HTML tags to parse when attribute exists
            'tagAttributes' => ['v-translate'],
            // Comments
            'commentAttributes' => ['translate-comment'],
            'contextAttributes' => ['translate-context'],
            // Attribute with plural content
            'pluralAttributes' => ['translate-plural'],
        ];
        ...
        if ($script) {
            ...
            static::getScriptTranslationsFromString($script, $translations, $options, $scriptLineNumber - 1);
        }
        ...
        if ($template) {
            static::getTemplateTranslations($template, $translations, $options, $template->getLineNo() - 1);
        }
    }
    /**
    ...
    protected static function getTagTranslations(array $options, DOMNode $dom, $translations)
    {
        ...
        for ($i = 0; $i < $children->length; $i++) {
            ...
            $translatable = \false;
            if (\in_array($node->tagName, $options['tagNames'], \true)) {
                $translatable = \true;
            }
