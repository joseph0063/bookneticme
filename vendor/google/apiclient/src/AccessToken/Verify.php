 ...
 */
namespace BookneticVendor\Google\AccessToken;

use BookneticVendor\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use BookneticVendor\Firebase\JWT\SignatureInvalidException;
use BookneticVendor\Firebase\JWT\Key;
use BookneticVendor\GuzzleHttp\Client;
use BookneticVendor\GuzzleHttp\ClientInterface;
use InvalidArgumentException;
use BookneticVendor\phpseclib3\Crypt\PublicKeyLoader;
use BookneticVendor\phpseclib3\Crypt\RSA\PublicKey;
use BookneticVendor\Psr\Cache\CacheItemPoolInterface;
use BookneticVendor\Google\Auth\Cache\MemoryCacheItemPool;
use BookneticVendor\Google\Exception as GoogleException;
use DateTime;
use DomainException;
use Exception;
use BookneticVendor\ExpiredException;
// Firebase v2
use LogicException;
/**
...
class Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('BookneticVendor\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'BookneticVendor\\Firebase\\JWT\\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (\class_exists('BookneticVendor\\phpseclib3\\Crypt\\RSA\\PublicKey')) {
    ...
    {
        if (\class_exists('BookneticVendor\\phpseclib3\\Crypt\\RSA')) {
            return 'BookneticVendor\\phpseclib3\\Crypt\\RSA';
        }
        if (\class_exists('BookneticVendor\\phpseclib\\Crypt\\RSA')) {
            return 'BookneticVendor\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('BookneticVendor\\phpseclib3\\Math\\BigInteger')) {
            return 'BookneticVendor\\phpseclib3\\Math\\BigInteger';
        }
        if (\class_exists('BookneticVendor\\phpseclib\\Math\\BigInteger')) {
            return 'BookneticVendor\\phpseclib\\Math\\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (\class_exists('BookneticVendor\\phpseclib3\\Crypt\\AES')) {
            return 'phpseclib3\\Crypt\\AES::ENGINE_OPENSSL';
        }
        if (\class_exists('BookneticVendor\\phpseclib\\Crypt\\RSA')) {
            return 'phpseclib\\Crypt\\RSA::MODE_OPENSSL';
        }
        if (\class_exists('BookneticVendor\\Crypt_RSA')) {
