...
namespace BookneticVendor\GuzzleHttp\Handler;

use BookneticVendor\GuzzleHttp\Exception\ConnectException;
use BookneticVendor\GuzzleHttp\Exception\RequestException;
use BookneticVendor\GuzzleHttp\Promise as P;
use BookneticVendor\GuzzleHttp\Promise\FulfilledPromise;
use BookneticVendor\GuzzleHttp\Promise\PromiseInterface;
use BookneticVendor\GuzzleHttp\Psr7\LazyOpenStream;
use BookneticVendor\GuzzleHttp\TransferStats;
use BookneticVendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \BookneticVendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \BookneticVendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
