...
class AppointmentRequestData
{
    ...
    public function validateCustomerData()
	{
        ...
        if( ! empty( $this->customerData['email'] ) )
        {
            if ( ! CustomerHelper::canBeCustomer( $this->customerData['email'] ) )
            {
            ...
            if ( $wpUserId > 0 )
            {
                ...
                if (
                    $checkCustomerExists &&
                    ( ! ( Permission::isStaff() || Permission::isAdministrator() ) && $checkCustomerExists->email != $this->customerData['email'] )
                )
                {
