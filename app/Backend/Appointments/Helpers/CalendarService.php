...
class CalendarService extends ServiceDefaults
{
    ...
    private function getStaffCalendar(): array
    {
        ...
        return apply_filters( 'date_time_calendar_data', $this->calendarData, [ 'staffId' => $this->staffId, 'locationId' => $this->locationId, 'serviceId' => $this->serviceId ] );
    }
    ...
    private function initTimeslotByTime( DateTime &$j )
    {
        ...
        if( ! $match[ 'appointment' ] || $match[ 'can_book' ] )
        {
            ...
            // this hook will enable us to disable specific timeslots in the booking panel if needed
            $slotDisabled = apply_filters( 'bkntc_calendar_service_disable_slot', false, $start, $end, $this );
