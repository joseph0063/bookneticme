...
class LocalizationService
{
    ...
    public static function changeLanguageIfNeed()
    {
        ...
        add_filter( 'pre_determine_locale', fn( $locale ) => $defaultLng ?? $locale );

        load_textdomain( 'booknetic', self::getMoFile( $defaultLng ), $defaultLng );
    }
    ...
    public static function setLanguage( $language , $slug = 'booknetic' )
    {
        ...
        add_filter( 'pre_determine_locale', fn( $locale ) => $language ?? $locale );

        load_textdomain( $slug, self::getMoFile( $language , false , $slug ), $language );
    }
